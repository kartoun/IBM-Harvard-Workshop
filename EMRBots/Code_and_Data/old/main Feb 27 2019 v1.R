...
#Count total number of all patients of interest and their assocaited admissions. This number is basically equals to the total number of admissions with malignant neoplasm.
...
#Add another column to the data frame. The new column contains a unique combination of patient id and admission id.
...
#Calculating age and addting a new column to the data frame.
d.cohort <- d.cohort %>%
...
#How many patients are 18 years or older at the time of each admission?
nrow(d.cohort[d.cohort$PatientAge >= 18, ])

#How races are distributed?
summary(d.cohort$PatientRace)

#Let start constructing "Table 1".
...
#Incorporating most recent lab values per admission. Note that the table "MostRecentLabsPerAdmission.txt" was created using a seperated program (written in SQL). The code for this program is available.
